...
router.post('/', function(request, response){
    ...
    var gifurl = request.body.gifurl;
    ...
    if(err) {
        ...
        db.query('INSERT INTO feed ( feedid, createdon, title, gifurl, authorid) VALUES($1, $2, $3, $4, $5)', [...values], (err, table) => {
            if(err) {
...
// UPDATE request
// router.patch('/:feedid', function(request, response) {
...
// DELETE request
router.delete('/:feedid', function(request, response) {
    var id = request.params.feedid;
    pool.connect((err, db, done) => {
    if(err) {
        return response.status(400).send(err)
    }
    else {
        db.query('DELETE FROM feed WHERE feedid = $1', [id], (err, result) => {
            if(err) {
                return response.status(400).send(err)
            }
            else {
...
router.get('/', function(request, response) {
    ...
    if(err) {
        ...
        db.query('SELECT * FROM feed', (err, table) => {
            // done();
            if(err) {
                return response.status(400).send(err)
            }
